...
- Include `data-title` attribute on `<html>` tag
...
## Make It Memorable, Not Just Correct

### Achievements Must Feel Tangible
Numbers in circles are forgettable. Make achievements feel like real rewards:
- **Streaks**: Growing flames, stacking coins, rising plants, filling jars—something that *grows*
...
- Give metrics a physical metaphor users can feel

### States Must Look Different
Completed vs pending should be *obvious* at a glance:
- Completed items: subtle glow, checkmark animation, reduced opacity, gentle strikethrough
- Pending items: full presence, slight pulse or shimmer to invite interaction
- Don't rely on tiny checkboxes—make the entire card transform

### Hero Typography Must Command Attention
The main greeting/title should be the most memorable element:
...
- "Good morning" should feel like a warm hug, not a label
...
- Completion celebrations: confetti, particle bursts, satisfying scale-bounce
- Micro-interactions: buttons that squish, toggles that snap
- Loading states that feel alive, not just spinning

## Navigation: Be Intentional

Choose navigation that fits the app's purpose:

**Content-first apps** (TikTok, Instagram, YouTube): Standard bottom tab bar works—users need quick switching between feeds. Keep it simple and out of the way.

**Utility/productivity apps**: Consider minimal approaches—floating pill, single FAB, or hidden nav.

**Premium/luxury apps**: Floating blurred dock, asymmetric layout, or gesture-based navigation.

The problem isn't tab bars—it's the *default* 5-icon center-elevated pattern applied without thought. If a standard tab bar fits, use it. If not, explore alternatives.
...
- Navigation patterns applied without considering the app type
- Tiny checkboxes as the only state indicator
- Flat backgrounds with no texture
- Generic "Good morning, User" without personality
- Safe, forgettable typography choices

## Mobile Screen Template (iPhone 14 Pro: 393×852)
