...
export function summarizeHtml(html: string, maxAttrLength = 30): string {
  ...
  let attrCount = 0
  const truncatedAttrs = attributesStr
  ...
  if (textContent.length > 0) {
    const truncatedText = textContent.length > 30 ? textContent.slice(0, 30) + "..." : textContent
 ...
 *   in main
 */
export function formatElementWithAncestry(html: string, ancestorPath?: string[]): string {
