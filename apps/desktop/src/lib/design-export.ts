...
export async function downloadAsZip(designs: DesignFile[], zipName = "designs") {
  ...
  try {
    ...
    toast.success(
      `Exported ${designs.length} screen${designs.length > 1 ? "s" : ""} to ${zipName}.zip`,
    )
  } catch (err) {
...
export function copyMultipleAsHtml(designs: DesignFile[]) {
  ...
  const combined = designs.map((d) => `<!-- ${d.title} -->\n${d.html}`).join("\n\n")
...
export async function generatePngToPath(
  ...
  await bridge.fs.writeFile(outputPath, bytes)
}
...
export async function exportAsPng({
  ...
  const filePath = await bridge.dialog.save({
