  ...
  useEffect(() => {
    ...
    if (isQuestionStuck && isPermissionStuck) {
      ...
      if (questionEarliest <= permissionEarliest) {
        ...
        type = "permission"
        toolName =
          runningPermissionTools.find((t) => t.startTime === permissionEarliest)?.tool ?? null
      }
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex flex-col gap-1 min-w-0">
          <span className="text-sm font-medium text-amber-500">{message.title}</span>
          <span className="text-xs text-amber-500/80">{message.description}</span>
        </div>
      </div>
    </div>
  )
