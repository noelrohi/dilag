...
export function AgentSelectorButton({ className }: AgentSelectorButtonProps) {
  const { agents, selectedAgent, selectedAgentInfo, selectAgent, isLoading } = useAgents()
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" className={cn(selectorStyles, className)}>
          <span className="capitalize font-medium">{selectedAgentInfo?.name ?? "Agent"}</span>
      ...
      <DropdownMenuContent align="start">
        {agents.map((agent) => (
          ...
          >
            {agent.color && (
              <div className="size-2 rounded-full" style={{ backgroundColor: agent.color }} />
            )}
