...
const ProviderScreenRefsContext = createContext<ScreenReferencesContext | null>(null)
...
const useOptionalProviderScreenRefs = () => useContext(ProviderScreenRefsContext)
...
export const usePromptInputScreenRefs = () => {
  ...
  if (!provider) {
    throw new Error("usePromptInputScreenRefs must be used within a PromptInputProvider")
  }
  return provider
}
...
export function PromptInputScreenReference({
  ...
  return (
    ...
    >
      ...
      {elementLabel && (
        <span className="ml-0.5 text-[10px] font-medium text-primary/70">({elementLabel})</span>
      )}
      <button
...
export type PromptInputScreenReferencesProps = Omit<HTMLAttributes<HTMLDivElement>, "children"> & {
