...
export function useSendToChat() {
  const setPendingMessage = useChatInterfaceStore((state) => state.setPendingMessage)

  return (errorText: string, prefix = "I'm getting the following server error:\n\n") => {
...
export function usePendingMessage() {
  ...
  const clearPendingMessage = useChatInterfaceStore((state) => state.setPendingMessage)
